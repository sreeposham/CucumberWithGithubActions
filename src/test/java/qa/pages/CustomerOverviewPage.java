...
public class CustomerOverviewPage extends BasePage {
	...
	public CustomerOverviewPage(WebDriver driver) {
		super(driver);
	}
...
//			By byAlert=By.xpath("//div[@class='SnackbarItem-message'][.='"+msgText+"']");
	...
	public void clickTabMenu(String menu) {
		if(menu==null) return;
			click(getElement(menu));
	}

	public void click_WalletManagement(String btn) {
		if(btn==null) return;
			switch(btn.toUpperCase()) {
			case "DIRECT BANK TOPUP": 
				click(getElement(btn));
				break;
			case "CREDIT WALLET": 
				click(getElement(btn));
				break;
			case "DEBIT WALLET": 
				...
				break;
			default:
				Assert.fail("Incorrect button name provided");
			}
	}
